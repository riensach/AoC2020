...
$puzzleInput = "eyr:2029 byr:1931 hcl:z cid:128
...
$validPassports = 0;
foreach($inputArray as $key => $value) {
    $presentValues = explode(" ",$value);
    $validCount = 0;
    foreach($presentValues as $key1 => $value1) {
        $valuePieces = explode(":",$value1);
        $id = $valuePieces[0];
        ...
        if($id=='cid') {
            ...
            $validCount++;
        } elseif($id=='eyr' && $valueInfo >=2020 && $valueInfo <=2030) {
            $validCount++;
        } elseif($id=='hgt') {
            $lastTwo = substr($valueInfo,-2,2);
            if($lastTwo=='cm') {
                $firstThree = substr($valueInfo,0,3);
                if($firstThree >=150 && $firstThree <= 193) {
                    $validCount++;
                }
            } elseif($lastTwo=='in') {
                $firstTwo = substr($valueInfo,0,2);
                if($firstTwo >=59 && $firstTwo <= 76) {
                    $validCount++;
                }
            }
