...
//$puzzleInput = "mxmxvkd kfcds sqjhc nhms (contains dairy, fish)
//trh fvjkl sbzzf mxmxvkd (contains dairy)
//sqjhc fvjkl (contains soy)
//sqjhc mxmxvkd sbzzf (contains fish)";


$foodInputArray = explode("\n",$puzzleInput);
$foodArray = array();
$ingredientsArray = array();
$allergyArray = array();
$foodIngredientAllergyRelationshipArray = array();
foreach($foodInputArray as $key => $value) {
    $foodString = explode("(", $value);

    $foodString[1] = str_replace("contains ","",$foodString[1]);
    $foodString[1] = str_replace(")","",$foodString[1]);
    $foodString[1] = trim($foodString[1]);
    $foodContents = trim($foodString[0]);
    $allergyInputArray = explode(", ", $foodString[1]);
    foreach($allergyInputArray as $key2 => $value2) {
        if(!isset($allergyArray[$value2])) {
            $allergyArray[$value2] = 1;
        } else {
            $allergyArray[$value2] = $allergyArray[$value2] + 1;
        }
    }
    $ingredientsInputArray = explode(" ", $foodContents);
    foreach($ingredientsInputArray as $key2 => $value2) {
        if(!isset($ingredientsArray[$value2])) {
            $ingredientsArray[$value2] = 1;
        } else {
            $ingredientsArray[$value2] = $ingredientsArray[$value2] + 1;
        }
    }

    foreach($allergyInputArray as $key2 => $value2) {
        $foodContains = trim($value2);
        //echo "Food $foodContents contains $foodContains<br>";
        $foodArray[] = array('food' => $foodContents, 'contains' =>$foodContains);
    }
}

$possibleAllergyRelationship = array();
foreach($foodArray as $key => $value) {
    $firstFoods = explode(" ", $value['food']);
    $possibleFoodCombos = $firstFoods;
    foreach($foodArray as $key2 => $value2) {
        if($value2['contains'] <> $value['contains']) {
            continue;
        }
        $comparisonFoods = explode(" ", $value2['food']);

        $possibleFoodCombos = array_intersect($possibleFoodCombos,$comparisonFoods);

    }
    $possibleAllergyRelationship[$value['contains']] = $possibleFoodCombos;

}
array_multisort(array_map('count', $possibleAllergyRelationship), SORT_ASC, $possibleAllergyRelationship);
//ksort($possibleAllergyRelationship);

var_dump($possibleAllergyRelationship);
$validOption = array();
$sizeOfOptions = count($possibleAllergyRelationship);
for ($x = 0; $x < $sizeOfOptions; $x++) {
    $arrayValueIndex = current($possibleAllergyRelationship);
    $arrayValueKey = key($possibleAllergyRelationship);
    $arrayValue = current($arrayValueIndex);
    $validOption[$arrayValueKey] = $arrayValue;
    $possibleAllergyRelationship = removeOption($possibleAllergyRelationship,$arrayValue);
    array_multisort(array_map('count', $possibleAllergyRelationship), SORT_ASC, $possibleAllergyRelationship);

    while(array_key_last($possibleAllergyRelationship) <> key($possibleAllergyRelationship) && count(current($possibleAllergyRelationship)) < 1) {
        next($possibleAllergyRelationship);
    }
}
function removeOption($remainingValidOptions,$toRemove):array {
    foreach ($remainingValidOptions as $key => $value) {
        foreach ($value as $key2 => $value2) {
            if($value2==$toRemove) {
                unset($remainingValidOptions[$key][$key2]);
            }
        }
    }
    return $remainingValidOptions;
}

//var_dump($foodArray);
//var_dump($ingredientsArray);
////var_dump($allergyArray);
//var_dump($possibleAllergyRelationship);
//var_dump($validOption);

$validIngredient = 0;
foreach($ingredientsArray as $key => $value) {

    if(!array_search($key,$validOption)) {
        // Not in the array, so count it
        $validIngredient+= $value;
    }

}

echo "We have $validIngredient valid ingredient occurances<Br>";
