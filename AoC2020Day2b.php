...
foreach($inputArray as $key => $value) {
    ...
    $lineValue = explode(":",$value);
    $stringValue = trim($lineValue[1]);
    $range = explode(" ",$lineValue[0]);
    $letter = $range[1];

    $rangeValues = explode("-",$range[0]);
    $lowRangeValue = $rangeValues[0];
    $highRangeValue = $rangeValues[1];

    //$character1 = substr($stringValue,$lowRangeValue-1,1);
    $character1 = $stringValue[$lowRangeValue-1];
    //$character2 = substr($stringValue,$highRangeValue-1,1);
    $character2 = $stringValue[$highRangeValue-1];


    //$foundLetterCount = substr_count($stringValue,$letter);

   // echo "String: $stringValue :: Letter: $letter :: Range: ".$rangeValues[0]." - ".$rangeValues[1]." - $character1 - $character2<br>";

    if($character1 == $letter xor $character2 == $letter) {
        // Valid PW
        $validPasswords++;
    }
...
echo "Total valid passwords: $validPasswords";