...
$receivedMessagesInput = "aaabbaababbababbabaabbbaaabbbbaa
...
$rulesArray = explode("\n",$rulesInput);
$messagesArray = explode("\n",$receivedMessagesInput);
$rules = array();
foreach($rulesArray as $key => $rulesValue) {
    $rulesValue = str_replace('"',"",$rulesValue);
    $rulesValue = explode(":",$rulesValue);

    $differentRulesArray = explode("|",$rulesValue[1]);
    $smallRulesArray = array();
    foreach($differentRulesArray as $rulesKey => $individualRulesValue) {
        $individualRulesValue = str_replace(' ', ',', trim($individualRulesValue));
        $smallRulesArray[] = $individualRulesValue;
    }

    $rules[$rulesValue[0]] = $smallRulesArray;
}
ksort($rules);
var_dump($rules);
$validMessages = 0;
foreach($messagesArray as $key => $messageValue) {
    $characters = str_split($messageValue,1);
    $validMessage = 1;
    if(count($characters) > count($rulesArray)) {
        // Too many characters, invalid, continue
        //$validMessage = 0;
        //continue;
        // Not sure this is the valid way of checking
    }
    foreach($characters as $charKey => $charValue) {
        // Logic for checking the message is valid
    }
    if($validMessage==1) {
        $validMessages++;
    }
...
for ($i = 0; $i < 10; $i++) {
    foreach ($rules as $key => $rulesValue) {
        $rulesCount = count($rulesValue);
        if ($rulesCount == 1 && ($rulesValue[0] == 'a' || $rulesValue[0] == 'b')) {
            // It's already a literal, time to move on
            continue;
        } elseif ($rulesCount == 1 && !strpos($rulesValue[0], ',')) {
            // It's 1 rule and only one possible option
        } elseif ($rulesCount == 1 && strpos($rulesValue[0], ',')) {
            // It's 1 rule and multiple options
            $ruleOptions = explode(",", $rulesValue[0]);
            foreach ($ruleOptions as $ruleOptionKey => $ruleOptionValue) {
                if($ruleOptionValue == 'a' || $ruleOptionValue == 'b') {
                    continue;
                }
                $rulesOptionCount = count($rules[$ruleOptionValue]);
                if ($rulesOptionCount == 1 && ($rules[$ruleOptionValue][0] == 'a' || $rules[$ruleOptionValue][0] == 'b')) {
                    // The sub-option is only one value
                    $newRuleValue = str_replace($ruleOptionValue, $rules[$ruleOptionValue][0], $rules[$key][0]);
                    $rules[$key][0] = $newRuleValue;
                } elseif ($rules[$ruleOptionValue][0] == 'a' || $rules[$ruleOptionValue][0] == 'b') {
                    $newRuleValue = str_replace($ruleOptionValue, $rules[$ruleOptionValue][0], $rules[$key][0]);
                    $rules[$key][0] = $newRuleValue;
                    echo "hi";
                }
            ...
            // It's multiple rules and multiple options
            foreach ($rulesValue as $rulesKey => $rulesSubValue) {
                $ruleOptions = explode(",", $rulesSubValue);
                foreach ($ruleOptions as $ruleOptionKey => $ruleOptionValue) {
                    if($rules[$ruleOptionValue][0] == 'a' || $rules[$ruleOptionValue][0] == 'b') {
                        continue;
                    }
                    $rulesOptionCount = count($rules[$ruleOptionValue]);
                    if ($rulesOptionCount == 1 && ($rules[$ruleOptionValue][0] == 'a' || $rules[$ruleOptionValue][0] == 'b')) {
                        // The sub-option is only one value
                        $newRuleValue = str_replace($ruleOptionValue, $rules[$ruleOptionValue][0], $rules[$key][$rulesKey]);
                        $rules[$key][$rulesKey] = $newRuleValue;
                    } elseif ($rules[$ruleOptionValue][0] == 'a' || $rules[$ruleOptionValue][0] == 'b') {
                        $newRuleValue = str_replace($ruleOptionValue, $rules[$ruleOptionValue][0], $rules[$key][$rulesKey]);
                        $rules[$key][$rulesKey] = $newRuleValue;
                        // echo "hi";
...
echo "We have $validMessages valid messages.<br>";
