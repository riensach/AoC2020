...
$puzzleInput = "LLLLL.LLLLLLLLLLLLLLLLLLLLLLLLLL.LLLLLLL.LLLLLLLLLL.LLLLLLLLLLL.LLLLLLLLLLLLLL.LLLL.LLLL.LLLLLL
...
while($iterator < 1000) {
    ...
    foreach($seatArray as $seatRowKey => $seatRowValue) {
        ...
        foreach ($seatRowValue as $seatColumnKey => $seatValue) {
            ...
            $seatColumnIterator = 2;
            while($seatLeft == '.') {
                $seatLeft = $seatArray[$seatRowKey][$seatColumnKey-$seatColumnIterator] ?? -1;
                $seatColumnIterator++;
            }
            $seatRight = $seatArray[$seatRowKey][$seatColumnKey+1] ?? -1;
            $seatColumnIterator = 2;
            while($seatRight == '.') {
                $seatRight = $seatArray[$seatRowKey][$seatColumnKey+$seatColumnIterator] ?? -1;
                $seatColumnIterator++;
            }
            $seatAbove = $seatArray[$seatRowKey-1][$seatColumnKey] ?? -1;
            $seatRowIterator = 2;
            while($seatAbove == '.') {
                $seatAbove = $seatArray[$seatRowKey-$seatRowIterator][$seatColumnKey] ?? -1;
                $seatRowIterator++;
            }
            $seatBelow = $seatArray[$seatRowKey+1][$seatColumnKey] ?? -1;
            $seatRowIterator = 2;
            while($seatBelow == '.') {
                $seatBelow = $seatArray[$seatRowKey+$seatRowIterator][$seatColumnKey] ?? -1;
                $seatRowIterator++;
            }
            $seatLeftUp = $seatArray[$seatRowKey-1][$seatColumnKey-1] ?? -1;
            $seatRowIterator = 2;
            $seatColumnIterator = 2;
            while($seatLeftUp == '.') {
                $seatLeftUp = $seatArray[$seatRowKey-$seatRowIterator][$seatColumnKey-$seatColumnIterator] ?? -1;
                $seatRowIterator++;
                $seatColumnIterator++;
            }
            $seatRightUp = $seatArray[$seatRowKey-1][$seatColumnKey+1] ?? -1;
            $seatRowIterator = 2;
            $seatColumnIterator = 2;
            while($seatRightUp == '.') {
                $seatRightUp = $seatArray[$seatRowKey-$seatRowIterator][$seatColumnKey+$seatColumnIterator] ?? -1;
                $seatRowIterator++;
                $seatColumnIterator++;
            }
            $seatLeftDown = $seatArray[$seatRowKey+1][$seatColumnKey-1] ?? -1;
            $seatRowIterator = 2;
            $seatColumnIterator = 2;
            while($seatLeftDown == '.') {
                $seatLeftDown = $seatArray[$seatRowKey+$seatRowIterator][$seatColumnKey-$seatColumnIterator] ?? -1;
                $seatRowIterator++;
                $seatColumnIterator++;
            }
            $seatRightDown = $seatArray[$seatRowKey+1][$seatColumnKey+1] ?? -1;
            $seatRowIterator = 2;
            $seatColumnIterator = 2;
            while($seatRightDown == '.') {
                $seatRightDown = $seatArray[$seatRowKey+$seatRowIterator][$seatColumnKey+$seatColumnIterator] ?? -1;
                $seatRowIterator++;
                $seatColumnIterator++;
            }

            $noOccupiedSeats = false;
            if($seatLeft <> '#' && $seatRight <> '#' && $seatAbove <> '#' && $seatBelow <> '#'
                && $seatLeftUp <> '#' && $seatRightUp <> '#' && $seatLeftDown <> '#' && $seatRightDown <> '#') {
                $noOccupiedSeats = true;
            }
            $occupiedSeatsGreaterThanFive = false;
            $occupiedSeatCount = 0;
            if($seatLeft=='#') $occupiedSeatCount++;
            if($seatRight=='#') $occupiedSeatCount++;
            if($seatAbove=='#') $occupiedSeatCount++;
            if($seatBelow=='#') $occupiedSeatCount++;
            if($seatLeftUp=='#') $occupiedSeatCount++;
            if($seatRightUp=='#') $occupiedSeatCount++;
            if($seatLeftDown=='#') $occupiedSeatCount++;
            if($seatRightDown=='#') $occupiedSeatCount++;
            if($occupiedSeatCount >= 5) {
                $occupiedSeatsGreaterThanFive = true;
            }

            if($seatValue == 'L' && $noOccupiedSeats) {
                $newSeatArray[$seatRowKey][$seatColumnKey] = '#';
            } elseif($seatValue == '#' && $occupiedSeatsGreaterThanFive) {
                $newSeatArray[$seatRowKey][$seatColumnKey] = 'L';
            }
        }
    }
...
$occupiedSeatCount = occupiedSeatCount($seatArray);
echo "Occupied seat count: $occupiedSeatCount";
