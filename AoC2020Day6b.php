...
$puzzleInput = "lznroxbqymvfijpwkec
...
foreach($inputArray as $key => $value) {
    ...
    foreach($people as $key2 => $value2) {
        // Logic here
        $value2 = trim($value2);
        $rowArrayInfo = str_split($value2,1);
        foreach($rowArrayInfo as $key4 => $value4) {
            if (isset($answeredYes[$value4])) {
                $answeredYes[$value4] = $answeredYes[$value4] + 1;
            } else {
                $answeredYes[$value4] = 1;
            }
        }
    }

    foreach($answeredYes as $key3 => $value3) {
        if($value3==$peopleCount) {
            // Everyone answered yes, count it
