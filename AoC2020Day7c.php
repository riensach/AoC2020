<?php
$puzzleInput = "posh blue bags contain 5 plaid chartreuse bags, 3 plaid lime bags.
...
//
//$puzzleInput = "dark lavender bags contain 1 posh teal bag.
//posh gray bags contain 5 vibrant yellow bags, 5 dotted salmon bags, 3 shiny tomato bags, 2 clear beige bags.
//dark gray bags contain 3 striped gray bags, 3 wavy tan bags.
//dark lime bags contain 1 dark salmon bag, 5 pale green bags, 2 striped green bags, 5 mirrored blue bags.
//vibrant black bags contain 3 light red bags, 4 plaid lime bags, 3 posh lime bags, 2 dotted lavender bags.
...
foreach($inputArray as $key => $value) {
    $bags = explode("contain",$value);
    $bags[0] = str_replace("bags","",$bags[0]);
    $bags[0] = trim($bags[0]);
    ...
    if($bags[1]=="no other bags.") {
        // Continue
        continue;
    } elseif(strpos($bags[1],",") === false) {
        // Just one bag
        $bags[1] = str_replace("bags","",$bags[1]);
        $bags[1] = str_replace("bag","",$bags[1]);
        $bags[1] = str_replace(".","",$bags[1]);
        $bags[1] = trim($bags[1]);
        ...
        // Multiple bags, explode by comma
        $bagsMulti = explode(",",$bags[1]);
        foreach($bagsMulti as $key2 => $value2) {
            $bagsToAdd = str_replace("bags","",$value2);
            $bagsToAdd = str_replace("bag","",$bagsToAdd);
            $bagsToAdd = str_replace(".","",$bagsToAdd);
   ...
   // var_dump($bagsToCheck);
    foreach($bagsToCheck as $key3 => $value3) {

        $bagArray[$bags[0]][$value3['color']] = $value3['count'];
    }
...
//        } else
        if(in_array($key, $legitBags)) {
            ...
            foreach ($value as $key2 => $value2) {
                if($key2=='shiny gold' || in_array($key2, $legitBags)) {
                    $legitBags[$key] = $key;

                    $legitCount = $legitCount+(int)$value2;
                }
            }
        }
    }
...
echo "Valid bags total is $countBags + $legitCount<br>";
